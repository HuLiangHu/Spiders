...
string sHost = oSession.host.ToLower();
            //test only
if (sHost == "search.weixin.qq.com")
{

    string sUrl = oSession.PathAndQuery;
    string sSession = oSession.ToString();
    string sMethod = oSession.RequestMethod.ToLower();
    if (sMethod == "get")
    {
        if (sUrl.StartsWith(@"/cgi-bin/searchweb/wxindex/querywxindexgroup"))
        {
        try
        {
            var openid = Regex.Match(sUrl, "openid=([^&]+)").Groups[1].Value;
            var search_key = Regex.Match(sUrl, @"search_key=([\d_]+)").Groups[1].Value;
            using (ConnectionMultiplexer redis = ConnectionMultiplexer.Connect("croton-redis-dev.redis.rds.aliyuncs.com"))
                {
            IDatabase db = redis.GetDatabase();
            db.StringSet("wechat:index:token", openid + " " + search_key);
        }
    }
