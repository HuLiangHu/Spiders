...
class MgtvSpider(scrapy.Spider):
    ...
    #详情页链接
    start_urls = 'https://www.mgtv.com/h/328819.html?cxid=95kqkw8n6'
...
##########################################
        ...
        yield scrapy.Request(url,callback=self.parse_prevideolink,meta={'albumurl':self.start_urls,'parmas':parmas})
    ...
    def parse_prevideolink(self, response):
        ...
        if int(parmas['page']) < total_page:
            ...
            url = self.videoapi + urlencode(parmas)
            yield scrapy.Request(url, callback=self.parse_prevideolink, meta={'albumurl':response.meta['albumurl'],'parmas': parmas})
        ...
        for info in json.loads(response.text)['data']['list']:
            ...
            try:
                ...
                if len(comment_times) < 3:
                    item['comment_time'] = str(time.strftime('%Y', time.localtime(ctime))) + '-' + '-'.join(
                        comment_times)
                else:
            ...
            item['ctime'] = str(datetime.now()).split('.')[0]
            yield item
        total_comment_count = json.loads(response.text)['data']['commentCount']
        total_comment_page = int(total_comment_count)//15 + 1
        comment_parmas=response.meta['comment_parmas']
        if int(total_comment_page)<100:
