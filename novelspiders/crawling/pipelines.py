        ...
        try:
            if item['name']:
                ...
                conn.execute("""
                    ...
                    item['description'][:2000], item['points'], item['yuepiao'], item['shoucang'], item['comment_count'],
                    ...
                    item['pubtime'], item['lastupdate'], item['current_date'], item['hongbao'], item['vipvote'],
                    item['printmark'], item['site']))
                # result = conn.fetchall()
        except Exception as e:
            logging.error("Insert item to Mysql faild.",e)
            logging.error('$' * 100)
            with open('omit.txt', 'a') as f:
                f.write(item['url'])
                f.write('\n')
