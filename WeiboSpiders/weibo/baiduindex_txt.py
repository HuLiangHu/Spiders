...
class BaiDuIndex(object):
    def __init__(self,keyword,cookie):
        ...
        self.day = 169 #查询时间
        self.startDate= datetime.datetime(2019,4,8)
        self.endDate =datetime.datetime(2019,4,16)
...
if __name__ == '__main__':
    # 蘑菇代理的隧道订单
    appKey = "bUVQZzdBYWZiUXl6QU9veDo0T1NoRUZyNXVWa0JFN0Q2"
    # 蘑菇隧道代理服务器地址
    ...
    cursor = conn.cursor()
    # cursor.execute("SELECT keyword FROM `tbl_baiduindex` WHERE `day`='2019-03-24' AND keyword not in (SELECT `keyword` FROM tbl_baiduindex WHERE `day`='2019-04-10')")
