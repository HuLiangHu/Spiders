...
#         movieid = '30139731'
#         start_urls = [
#             'https://movie.douban.com/subject/{0}/comments?sort=new_score&status=P'.format(
#                 str(movieid)),
#             # 看过热门
#             'https://movie.douban.com/subject/{0}/comments?sort=time&status=P'.format(
#                 str(movieid)),  # 看过最新
...
#所有热门
    def start_requests(self):
        ...
        :return:
        """
        url ='https://movie.douban.com/j/search_subjects?type=tv&tag=%E7%83%AD%E9%97%A8&sort=time&page_limit=500'
    ...
    def parse_detail(self, response):
        ...
        for content in contents:
            ...
            #print(item)
            yield item
        next_url = response.xpath('//div[@id="paginator"]/a[contains(text(),"后页")]/@href').extract_first()
        if next_url:
            yield scrapy.Request('https://movie.douban.com/subject/{}/comments'.format(response.meta['movieId']) + next_url,
                                 callback=self.parse_detail,meta={'name':response.meta['name'],'movieId': response.meta['movieId']}, dont_filter=True)
